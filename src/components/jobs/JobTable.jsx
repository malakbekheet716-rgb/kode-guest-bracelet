  ...
  return (
    <div className="table-wrap">
      <table className="table">
        <thead>
          <tr>
            <th>{t('jobDetail.jobId')}</th>
            <th>{t('jobDetail.quantity')}</th>
            <th>{t('jobDetail.status')}</th>
            <th>{t('jobDetail.createdAt')}</th>
          </tr>
        </thead>
        <tbody>
          {items.map((job) => (
            <tr key={job.id} className="is-clickable" onClick={() => navigate(`/jobs/${job.id}`)}>
              ...
              <td>
                ...
                {job.needsAttention && <span className="nav-link__dot" style={{ marginInlineStart: 8 }}>!</span>}
              </td>
